...
function paginateData() {
    ...
    if (startPage>11) {
     ...
     // 이전 버튼
    if (startPage>1) {
        paginationHtml += '<a href="javascript:void(0)" onclick="showPage(' + (currentPage - 10) + ')"><</a>';
    }
...
function showPage(page) {
    ...
    items.forEach(function(item) {
	    ...
	    htmlContent += '<td>';
	    htmlContent += '<a href="javascript:void(0)" onclick="moveMap(\'' + item.ADDR + '\', \'' + item.COMPANY_NAME + '\')">' + item.COMPANY_NAME +'</a>';
	    htmlContent += '</td>';
	    htmlContent += '<td>' + item.ADDR + '</td>';
	    htmlContent += '<td>' + item.OFFICE_TEL + '</td>';
	    htmlContent += '<td>' + item.SUPPORT + '</td>';
	    htmlContent += '</tr>';
    });
